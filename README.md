...
[![CI](https://github.com/n8mauer/Nexus/actions/workflows/ci.yml/badge.svg)](https://github.com/n8mauer/Nexus/actions/workflows/ci.yml)
...
The environment is built so that theory-of-mind behaviors (anticipating competitor bids, forming strategic coalitions, and detecting deceptive signaling) must be learned through Group Relative Policy Optimization (GRPO) training rather than hand-coded rules. Nexus integrates with the OpenEnv 0.2.1 framework via a ProxyAgent HTTP bridge, and the included training notebook sets up GRPO fine-tuning of Qwen2.5-1.5B with Unsloth 4-bit quantization and LoRA. Trained-agent evaluation results are not yet published in this repository; see the status section below for what is implemented versus planned.

---

## Demo

One command runs a seeded simulation: the `tiny` preset, one greedy agent against one random agent, seed 42, 10 rounds, finishing in about a second.
...
Output of the seeded run (deterministic for seed 42):

```text
Nexus demo: preset=tiny, agents=greedy vs random, seed=42
Running 10 rounds with 2 agents...

                                    Final Scoreboard
+--------------------------------------------------------------------------------------+
| Agent      | Policy | Score | Jobs Done | Jobs Missed | Trades | Budget | Reputation |
|------------+--------+-------+-----------+-------------+--------+--------+------------|
| Team Alpha | greedy |   826 |         6 |           1 |      2 |  $1670 |         57 |
| Team Beta  | random |   456 |         2 |           1 |      2 |   $312 |         53 |
+--------------------------------------------------------------------------------------+

Rounds played: 10
Social welfare: 1281
Gini coefficient: 0.144
Market volume: $892 across 2 trades
Journal written to demo_output (journal.md, metrics.jsonl, final_results.json)
```

![Recorded terminal output of python demo.py](docs/images/demo.svg)

The demo writes `journal.md`, `metrics.jsonl`, and `final_results.json` to `demo_output/`. `tests/test_demo.py` asserts this exact contract, and CI runs both the test suite and the demo on every push (acceptance Gates 1 and 2 in [docs/ACCEPTANCE.md](docs/ACCEPTANCE.md)).
...
- **Emergent strategy:** Coalition formation, bluffing, and reputation dynamics can arise from the market mechanics rather than being scripted.
...
├── requirements.lock            # Locked dependencies (used by CI)
├── openenv.yaml                 # OpenEnv environment manifest
├── demo.py                      # Seeded demo entry point (wraps scripts/demo.py)
├── nexus/
│   ├── __init__.py
│   ├── state.py                 # Resource, Job, AgentState, ClusterState, MarketState
│   ├── config.py                # NexusConfig, presets (tiny/standard/large/oversight/multi_actor)
...
├── nexus_openenv/
│   ├── client.py                # NexusEnv OpenEnv client
│   ├── models.py                # NexusAction, NexusObservation
│   └── server/                  # FastAPI server (ProxyAgent HTTP bridge)
├── notebooks/
│   └── nexus_training.ipynb     # GRPO fine-tuning setup (Unsloth + TRL)
├── deploy/space/                # Hugging Face Space overlay files
├── scripts/
│   ├── run_simulation.py        # Typer CLI: run, evaluate
│   ├── demo.py                  # Seeded demo implementation (Gate 2)
│   ├── openenv_smoke.py         # OpenEnv server round-trip check (Gate 3)
│   ├── evaluate.py              # Metric computation + comparison
│   ├── visualize.py             # Rich terminal replay
│   └── build_space.py           # Assemble the HF Space staging directory
├── .github/workflows/ci.yml     # CI: test suite, demo, server smoke test
└── tests/
    ├── test_engine.py
    ├── test_market.py
    ├── test_coalitions.py
...
### Dependencies
- Python 3.11+
- `pydantic` for state and action validation
- `openenv-core`, `fastapi`, `uvicorn` for the OpenEnv server integration
- `anthropic` for the LLM agent backend
- `scikit-learn` for the CART oversight probes
- `rich` and `typer` for the CLI and terminal visualization
- `pytest` for testing (dev extra)
...
# Run the seeded demo (tiny preset, greedy vs random, seed 42)
python demo.py

# Run a simulation from the CLI
python -m scripts.run_simulation run --preset standard --agents greedy,random,greedy,random

# Start the OpenEnv server
uvicorn nexus_openenv.server.app:app --host 0.0.0.0 --port 8000

# Check the server round trip (health, reset, one step)
python scripts/openenv_smoke.py

# Rebuild the Hugging Face Space staging directory
...
**Results provenance:** for exactly which results are verifiable from this repository versus self-reported or resume-only, see [docs/RESULTS.md](docs/RESULTS.md). No trained-agent results exist in this repository yet.
...
### Implemented
- Core engine: state dataclasses (`nexus/state.py`), action parsing and validation (`nexus/actions.py`), the 7-phase round loop (`nexus/engine.py`), job generation and deadline tracking, observation rendering (`nexus/observations.py`), and reward computation (`nexus/rewards.py`)
- Market and negotiation: bid/offer matching with price impact and commission (`nexus/market.py`), free-text message passing, coalition proposal and MoE voting (`nexus/coalitions.py`), and random events (`nexus/events.py`)
- Agents: random and greedy baselines, an Anthropic tool-use LLM agent (`agents/llm_agent.py`), an MCTS/UCB1 strategic agent, a CART-probe supervisor, and a directive-based CTO agent
- Oversight: privileged supervisor view, supervisor action interface, and behavioral probes (`nexus/oversight.py`)
- Multi-actor mode: CTO directives and worker agents with a noise/misinterpretation model (`nexus/multi_actor.py`)
- OpenEnv 0.2.1 integration: HTTP server, client, and ProxyAgent bridge (`nexus_openenv/`), covered by `tests/test_openenv.py`
- Tooling: Typer CLI with `run` and `evaluate` commands, Rich terminal replay, simulation journaling and persistence, a seeded demo (`demo.py`), an OpenEnv server smoke script (`scripts/openenv_smoke.py`), a 42-test suite, and a CI workflow that enforces the acceptance gates (`.github/workflows/ci.yml`)

### Planned
- Scripted anomaly injection scenarios (collusion, hoarding) for supervisor training
- Gymnasium environment wrapper
- Evaluation leaderboard
- Published GRPO training runs with trained-agent evaluation results (the training notebook currently documents the setup, not completed runs)

---

## Product Documentation

- [Product requirements (PRD)](docs/PRD.md): personas, workflow, problem, success metrics, non-goals
- [Architecture decision records](docs/decisions/): observation and action design, CART oversight probes, generated Space staging
- [Roadmap](ROADMAP.md): milestones with status and evidence
- [Acceptance criteria](docs/ACCEPTANCE.md): the gates CI and local checks enforce
- [Results provenance](docs/RESULTS.md): what is verified here versus self-reported or resume-only
...
4. **Market Physics from Portfolio Simulation:** Commission, price impact, and portfolio accounting create realistic market dynamics: large trades move prices, commissions discourage churn, and Sharpe ratio provides risk-adjusted evaluation.
...
- **A single environment** that combines multi-agent LLM training, resource negotiation, behavioral oversight, and GRPO fine-tuning. We have not found another framework that unifies all four.
- **MCTS for negotiation planning.** We are not aware of an existing tool that applies Monte Carlo Tree Search to multi-agent resource negotiation with hidden information.
- **Active behavioral oversight.** CART ensemble probes classify agent strategies (collusion, hoarding, free-riding) in real time, going beyond passive observability.
- **Theory-of-mind as a training target.** The mechanics are designed so that bid anticipation, coalition strategy, and counter-manipulation must be learned rather than hand-coded, in line with current theory-of-mind research.
- **Direct simulation-to-reality mapping.** GPU and CPU resources map to EC2 instance types, job deadlines to SLA workloads, and market mechanics to spot pricing dynamics.

### Key Risks

- **Simulation-to-reality gap.** Real cloud markets have API rate limits, multi-region pricing, and contractual constraints not yet modeled.
- **Well-funded incumbents.** Spot by NetApp and Pactum AI have production deployments and enterprise sales teams; they could add agent-based negotiation to existing platforms.
- **Scale validation needed.** Current testing covers 2 to 4 agents over 10 to 50 rounds; behavior with 8 or more agents or over hundreds of rounds is untested.

## 14. Citing Nexus

If you use Nexus in your research, please cite this repository:

```bibtex
@software{mauer2026nexus,
  title={Nexus: A Multi-Agent Compute Cluster Negotiation Environment},
  author={Mauer, Nate},
  year={2026},
  url={https://github.com/n8mauer/Nexus}
}
```
